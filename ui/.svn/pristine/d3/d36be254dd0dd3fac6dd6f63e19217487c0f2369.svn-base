...
(function() {
    ...
    angular
        .module('LLDDHealthProblemCategoryService', ['cid.app.constants', 'ui-notification'])
        .factory('LLDDHealthProblemCategory', lLDDHealthProblemCategoryFactory);

    lLDDHealthProblemCategoryFactory.$inject = ['$q', '$http', '$rootScope', 'GLOBAL', 'Notification'];

    function lLDDHealthProblemCategoryFactory($q, $http, $rootScope, GLOBAL, Notification) {
        ...
        function getAll() {
            ...
            $http.get(url).then(function(response) {
                ...
                $rootScope.$emit('lLDDHealthProblemCategory-loaded', response.data);
            }, function(response) {
        ...
        function create(lLDDHealthProblemCategory, callback) {
            var deferred = $q.defer();
            if (lLDDHealthProblemCategory != undefined && lLDDHealthProblemCategory != null) {
                $http.post(url, lLDDHealthProblemCategory).then(function(response) {
                    deferred.resolve(response);
                    $rootScope.$emit('lLDDHealthProblemCategory-saved', response.data);
                    if (callback) {
                        callback();
                    }
                }, function(response) {
                ...
                deferred.reject({
                    message: "No LLDDHealthProblemCategory ID Supplied"
                });
            }
            return deferred.promise;
        }
        ...
        function save(lLDDHealthProblemCategory, callback) {
            ...
            if (lLDDHealthProblemCategory && lLDDHealthProblemCategory.id) {
                $http.put(url + lLDDHealthProblemCategory.id, lLDDHealthProblemCategory).then(function(response) {
                    deferred.resolve(response);
                    $rootScope.$emit('lLDDHealthProblemCategory-saved', response.data);
                    if (callback) {
                        callback();
