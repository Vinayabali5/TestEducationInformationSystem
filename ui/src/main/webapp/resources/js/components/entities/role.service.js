...
(function() {
    ...
    angular
        .module('RoleService', ['cid.app.constants', 'ui-notification'])
        .factory('Role', roleFactory);

    roleFactory.$inject = ['$q', '$http', '$rootScope', 'GLOBAL', 'Notification'];

    function roleFactory($q, $http, $rootScope, GLOBAL, Notification) {
        ...
        function getAll() {
            ...
            $http.get(url).then(function(response) {
                ...
                $rootScope.$emit('roles-loaded', response.data);
            }, function(response) {
        ...
        function create(role, callback) {
            ...
            if (role != undefined && role != null) {
                $http.post(url, role).then(function(response) {
                    deferred.resolve(response);
                    $rootScope.$emit('role-saved', response.data);
                    if (callback) {
                        callback();
                    }
                }, function(response) {
        ...
        function save(role, callback) {
            var deferred = $q.defer();
            if (role && role.id) {
                $http.put(url + role.id, role).then(function(response) {
                    deferred.resolve(response);
                    $rootScope.$emit('role-saved', response.data);
                    if (callback) {
                        callback();
