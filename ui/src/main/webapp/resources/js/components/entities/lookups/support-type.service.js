...
(function() {
    ...
    angular
        .module('SupportTypeService', ['cid.app.constants', 'ui-notification'])
        .factory('SupportType', supportTypeFactory);

    supportTypeFactory.$inject = ['$q', '$http', '$rootScope', 'GLOBAL', 'Notification'];

    function supportTypeFactory($q, $http, $rootScope, GLOBAL, Notification) {
        ...
        function getAll() {
            ...
            $http.get(url).then(function(response) {
                ...
                $rootScope.$emit('support-types-loaded', response.data);
            }, function(response) {
        ...
        function create(supportType, callback) {
            var deferred = $q.defer();
            if (supportType != undefined && supportType != null) {
                $http.post(url, supportType).then(function(response) {
                    deferred.resolve(response);
                    $rootScope.$emit('support-types-saved', response.data);
                    if (callback) {
                        callback();
                    }
                }, function(response) {
                ...
                deferred.reject({
                    message: "No SupportType ID Supplied"
                });
            }
            return deferred.promise;
        }
        ...
        function save(supportType, callback) {
            ...
            if (supportType && supportType.id) {
                $http.put(url + supportType.id, supportType).then(function(response) {
                    deferred.resolve(response);
                    $rootScope.$emit('support-types-saved', response.data);
                    if (callback) {
                        callback();
